...
func (cfg Config) fromBase64() (config, error) {
	...
	if len(newCfg.AppID) != AppPublicKeySize {
		return config{}, fmt.Errorf("wrong byte size for AppID: %d, should be %d", len(newCfg.AppID), AppPublicKeySize)
	}
	...
	if len(newCfg.AppSecret) != AppSecretSize {
		return config{}, fmt.Errorf("wrong byte size for AppSecret: %d, should be %d", len(newCfg.AppSecret), AppSecretSize)
	}
